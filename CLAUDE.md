...
- **5-stage matching pipeline** (`matching/matcher.py` + `matching/verifier.py`): ground-truth lookup → rule filter (date proximity + keyword overlap) → semantic bi-encoder ranking (all-MiniLM-L6-v2) → NLI cross-encoder verification (nli-deberta-v3-small, bidirectional entailment) → Ollama LLM fallback (phi3:mini, for uncertain NLI results). Verification results cached in SQLite `match_verdicts` table. `MatchVerifier` is optional — without it, the matcher falls back to unverified semantic matches.
